...
def openDb():
    ...
    db = connector.connect(
            host="rst-db.mysql.database.azure.com",  # Host dari Azure
            user="adminadmin",                    # Format username: admin@nama-server
            password="Admin123",                    # Password database Anda
            database="aptk",                        # Nama database
    )
    if db.is_connected():
        print("Berhasil terhubung dengan database Azure")
...
def cart():
    ...
    cursor.execute("SELECT * FROM keranjang_pelanggan WHERE pelanggan=%s", (session['no_hp'],))
    result = cursor.fetchall()
    ...
    # Menghitung jumlah produk di keranjang (menggunakan parameterized query)
    if 'loggedin' in session:
        cursor.execute("SELECT COUNT(*) FROM cart WHERE pelanggan=%s", (session['no_hp'],))
        length = cursor.fetchone()[0]  # Mengambil nilai tunggal dari hasil query
    else:
        length = 0

    # Menghitung total harga dari keranjang
    cursor.execute("SELECT SUM(total_harga) AS grand_total FROM keranjang_pelanggan WHERE pelanggan=%s", (session['no_hp'],))
    grandtotal = cursor.fetchone()[0]  # Mengambil nilai tunggal dari hasil query
